...
CREATE POLICY "Teachers can manage own assignments"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.classes
      WHERE id = assignments.class_id AND teacher_id = (select auth.uid())
    ) AND
    EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = (select auth.uid()) AND role = 'teacher'
...
CREATE POLICY "Students can view class assignments"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.class_members
      WHERE class_id = assignments.class_id AND student_id = (select auth.uid())
    ) AND
    EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = (select auth.uid()) AND role = 'student'
...
CREATE POLICY "Students can update own submissions"
  ...
  TO authenticated
  USING (student_id = (select auth.uid()) AND status != 'graded')
  WITH CHECK (student_id = (select auth.uid()) AND status != 'graded');
...
CREATE POLICY "Teachers can manage own announcements"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.classes
      WHERE id = announcements.class_id AND teacher_id = (select auth.uid())
    ) AND
    EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = (select auth.uid()) AND role = 'teacher'
...
CREATE POLICY "Students can view class announcements"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.class_members
      WHERE class_id = announcements.class_id AND student_id = (select auth.uid())
    ) AND
    EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = (select auth.uid()) AND role = 'student'
...
CREATE POLICY "Teachers can manage own lectures"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.classes
      WHERE id = lectures.class_id AND teacher_id = (select auth.uid())
    ) AND
    EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = (select auth.uid()) AND role = 'teacher'
...
CREATE POLICY "Students can view class lectures"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.class_members
      WHERE class_id = lectures.class_id AND student_id = (select auth.uid())
    ) AND
    EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = (select auth.uid()) AND role = 'student'
